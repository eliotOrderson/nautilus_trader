...
if [[ "$EVENT_NAME" == "push" ]]; then
  ...
  # Try BEFORE_SHA first for incremental diff; fall back to upstream/develop
  # on force-push / history rewrite when BEFORE_SHA is orphaned
  if git rev-parse --quiet --verify "$BEFORE_SHA^{commit}" > /dev/null 2>&1; then
    changed_files="$(git diff --name-only "$BEFORE_SHA" HEAD)"
  else
    git fetch https://github.com/nautechsystems/nautilus_trader develop --depth=1
