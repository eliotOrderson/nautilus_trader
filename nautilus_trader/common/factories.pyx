...
cdef class OrderFactory:
    ...
    cpdef OrderList bracket(
        ...
        TriggerType tp_emulation_trigger = TriggerType.NO_TRIGGER,
        bint tp_reduce_only = True,
        ...
        TriggerType sl_emulation_trigger = TriggerType.NO_TRIGGER,
        bint sl_reduce_only = True,
    ):
        """
        ...
        tp_client_order_id : ClientOrderId, optional
            The custom client order ID for the order.
            If a client order ID is not provided then one will be generated by the factory.
        tp_emulation_trigger : TriggerType, default ``NO_TRIGGER``
            The emulation trigger type for the take-profit order.
        tp_reduce_only : bool, default True
            If the take-profit order will only reduce a position.
        ...
        sl_client_order_id : ClientOrderId, optional
            The custom client order ID for the order.
            If a client order ID is not provided then one will be generated by the factory.
        sl_emulation_trigger : TriggerType, default ``NO_TRIGGER``
            The emulation trigger type for the stop-loss order.
        sl_reduce_only : bool, default True
            If the stop-loss order will only reduce a position.
        ...
        if tp_order_type == OrderType.LIMIT:
            tp_order = LimitOrder(
                ...
                quote_quantity=quote_quantity,
                display_qty=None,
                emulation_trigger=tp_emulation_trigger,
                trigger_instrument_id=trigger_instrument_id,
        ...
        if sl_order_type == OrderType.STOP_MARKET:
            sl_order = StopMarketOrder(
                ...
                reduce_only=sl_reduce_only,
                quote_quantity=quote_quantity,
                emulation_trigger=sl_emulation_trigger,
                trigger_instrument_id=trigger_instrument_id,
                contingency_type=contingency_type,
                order_list_id=order_list_id,
